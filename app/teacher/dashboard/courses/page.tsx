...
export default function TeacherCourseManagementPage() {
  ...
  const fetchCourses = useCallback(async (id: string) => {
    // Use select("*") for backward compatibility with older table schemas.
    const { data: courseData, error } = await supabase.from("courses").select("*").eq("teacher_id", id);

    if (error) {
      const detail = [error.code, error.message].filter(Boolean).join(": ");
      setErrorMessage(`讀取課程清單失敗${detail ? `（${detail}）` : ""}`);
      return;
    }

    const normalizedCourses = ((courseData ?? []) as Record<string, unknown>[])
      .map((row) => ({
        id: String(row.id ?? ""),
        title: String(row.title ?? "未命名課程"),
        course_code: String(row.course_code ?? row.code ?? "未設定代碼"),
