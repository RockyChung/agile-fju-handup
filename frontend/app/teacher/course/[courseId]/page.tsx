...
type ScoreMode = "individual" | "group";

function formatTeacherHandRaiseLabel(reportOrder: string[], groupName: string | null, studentName: string): string {
  if (!groupName) {
    return studentName;
  }
...
export default function TeacherCoursePage() {
  ...
  const [pendingDeleteScore, setPendingDeleteScore] = useState<ScoreHistoryItem | GroupHistoryItem | null>(null);
  const [scoreMode, setScoreMode] = useState<ScoreMode>("individual");
  ...
  const dm = scoreMode === "group";

  const persistHandRaiseMode = useCallback(
    async (mode: ScoreMode) => {
      ...
      try {
        const response = await fetch(`${getBackendApiBaseUrl()}/courses/${courseId}`, {
          ...
          body: JSON.stringify({ handRaiseMode: mode }),
        });
        if (!response.ok) {
          setErrorMessage("無法儲存舉手／給分模式，請稍後再試。");
        }
      } catch {
        setErrorMessage("無法連線到後端服務。");
      }
    },
    [courseId, token],
  );
  ...
  const loadQueue = useCallback(
    async (authToken: string, options?: LoadQueueOptions) => {
      ...
      try {
        ...
        setScoreMode(json.course.handRaiseMode === "group" ? "group" : "individual");
        setReportOrder(json.course.reportOrder ?? []);
        if (json.course.handRaiseMode === "group") {
          const agr = json.activeGroupScoreRound ?? null;
          setActiveGroupScoreRound(agr);
          setGroupRoundId(agr?.id ?? null);
        } else {
          setActiveGroupScoreRound(null);
        ...
        setCurrentSpeaker(
          json.course.currentSpeaker
            ? {
                ...
                groupName: json.course.currentSpeaker.groupName ?? null,
              }
            : null,
        );
        setQueue(
  ...
  const openTeacherScoringModal = async () => {
    if (!currentSpeaker || !token || !courseId) {
      return;
    }
    setInputScore(0);
    setErrorMessage(null);
    if (scoreMode === "group") {
      const res = await fetch(`${getBackendApiBaseUrl()}/courses/${courseId}/group-score-rounds`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ speakerStudentId: currentSpeaker.studentId }),
      });
      if (!res.ok) {
        const err = (await res.json().catch(() => ({}))) as { message?: string };
        setErrorMessage(err.message || "無法開始給分流程。");
        return;
      }
      const created = (await res.json()) as { round?: { id: string } };
      if (created.round?.id) {
        setGroupRoundId(created.round.id);
      }
    }
    setScoringModalOpen(true);
  };

  const closeScoringModal = async () => {
    if (
      scoreMode === "group" &&
      groupRoundId &&
      token &&
      courseId &&
      !activeGroupScoreRound?.teacherSubmitted
    ) {
      await fetch(`${getBackendApiBaseUrl()}/courses/${courseId}/group-score-rounds/${groupRoundId}`, {
        method: "DELETE",
  ...
  const handleSubmitScore = async () => {
    ...
    setSavingScore(true);
    setErrorMessage(null);
    const scoreValue = Math.max(0, Math.min(100, Math.floor(inputScore)));

    if (scoreMode === "group") {
      const roundId = groupRoundId ?? activeGroupScoreRound?.id;
      if (!roundId) {
        setErrorMessage("給分流程尚未建立，請關閉後再試一次。");
        setSavingScore(false);
        return;
      }
      const response = await fetch(
        `${getBackendApiBaseUrl()}/courses/${courseId}/group-score-rounds/${roundId}/teacher`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteScore = async (scoreId: string, isGroupScore: boolean) => {
    if (!token || !courseId) {
      return;
    }
    setDeletingScoreId(scoreId);
    setErrorMessage(null);
    const path = isGroupScore
      ? `${getBackendApiBaseUrl()}/courses/${courseId}/group-scores/${scoreId}`
      : `${getBackendApiBaseUrl()}/courses/${courseId}/scores/${scoreId}`;
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className={dm ? "g-panel p-6" : "rounded-2xl border border-slate-100 bg-white p-6"}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className={`text-2xl font-black ${dm ? "g-heading" : "text-slate-800"}`}>
                {courseTitle} - 課程頁面
              </h1>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className={`text-sm font-semibold ${dm ? "text-slate-300" : "text-slate-600"}`}>個人／分組：</span>
                <button
                  type="button"
                  onClick={() => {
                    setScoreMode("individual");
                    void persistHandRaiseMode("individual");
                  }}
                ...
                <button
                  ...
                  onClick={() => {
                    setScoreMode("group");
                    void persistHandRaiseMode("group");
                  }}
                  className={`rounded-xl px-4 py-2 text-sm font-bold ${
                    scoreMode === "group"
                      ? "bg-indigo-600 text-white"
              ...
              <p className={`mt-1 text-xs ${dm ? "g-muted" : "text-slate-500"}`}>
                個人：每人可各自舉手；給分僅記錄發言代表。分組：同組僅一人可舉手；畫面為暗色主題；老師給分後各組組長會跳出評分視窗，發言組每位成員成績為「老師分數＋各組長分數加總」（單次上限 100 分）。
              </p>
            </div>
          ...
          <p className={`mt-3 text-sm font-semibold text-indigo-600 ${dm ? "text-indigo-300" : ""}`}>
            {queueCountText}
          </p>
          {dm && activeGroupScoreRound?.teacherSubmitted && currentSpeaker && (
            <p className="mt-3 rounded-xl border border-indigo-500/40 bg-indigo-950/40 px-4 py-3 text-sm font-semibold text-indigo-100">
              老師分數已送出，等待組長評分（{activeGroupScoreRound.leadersSubmitted}/
              {activeGroupScoreRound.leadersRequired}）。全部提交後將自動合計（老師＋組長）並寫入發言組每位成員（單次合計上限 100 分）。
            </p>
          )}
          {errorMessage && (
        ...
        <section className={dm ? "g-panel p-5" : "rounded-2xl border border-slate-100 bg-white p-5"}>
          <h2 className={`text-lg font-bold ${dm ? "g-heading" : "text-slate-800"}`}>置頂發言區塊</h2>
          {!currentSpeaker ? (
            <p
              className={
                dm
                  ? "g-panel-dashed mt-2 px-4 py-4 text-sm font-semibold g-muted"
                  : "mt-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-4 text-sm font-semibold text-slate-500"
              }
            >
              尚未選擇發言學生
            </p>
            ...
            <article
              className={
                dm
                  ? "mt-3 flex items-center justify-between rounded-2xl border border-indigo-500/30 bg-indigo-950/40 p-5"
                  : "mt-3 flex items-center justify-between rounded-2xl border border-indigo-100 bg-indigo-50 p-5"
              }
            >
              <div>
                <p className={`text-sm font-bold ${dm ? "text-indigo-300" : "text-indigo-600"}`}>目前發言</p>
                <h3 className={`text-xl font-black ${dm ? "g-heading" : "text-slate-800"}`}>
                  {scoreMode === "group"
                    ? formatTeacherHandRaiseLabel(
                        reportOrder,
                        currentSpeaker.groupName,
                        currentSpeaker.studentName,
                      )
                    : currentSpeaker.studentName}
                </h3>
                <p className={`mt-1 text-sm ${dm ? "g-muted" : "text-slate-600"}`}>{currentSpeaker.studentNo}</p>
                {scoreMode === "group" && !currentSpeaker.groupName && (
                  <p className={`mt-1 text-xs font-semibold ${dm ? "text-amber-300" : "text-amber-700"}`}>
                    此代表未設定組別，成績僅計入本人。
                  </p>
                )}
              </div>
              <button
                type="button"
                onClick={() => void openTeacherScoringModal()}
        ...
        <section className="grid gap-4 lg:grid-cols-[2fr_1fr]">
                ...
                >
                  <div>
                    ...
                    <h2 className={`text-lg font-bold ${dm ? "g-heading" : "text-slate-800"}`}>
                      {scoreMode === "group"
                        ? formatTeacherHandRaiseLabel(reportOrder, item.groupName, item.studentName)
                        : item.studentName}
                    </h2>
                    <p className={`mt-1 text-sm ${dm ? "g-muted" : "text-slate-500"}`}>{item.studentNo}</p>
                    {scoreMode === "individual" && (
                      <p className={`mt-1 text-xs font-semibold ${dm ? "text-slate-400" : "text-slate-600"}`}>
          ...
          <aside className={dm ? "g-panel p-5" : "rounded-2xl border border-slate-100 bg-white p-5"}>
            {dm ? (
              <>
                <h2 className="text-lg font-bold g-heading">當日組別得分版</h2>
                ...
                <div className="mt-3 space-y-2">
                  {groupScoreboard.length === 0 ? (
                    ...
                    groupScoreboard.map((row, index) => (
                      <article
                        key={row.groupKey}
                        className="rounded-xl border border-slate-600 bg-slate-800/80 px-3 py-2.5"
                      >
                ...
                <h3 className="mt-6 text-base font-bold g-heading">組別給分歷程</h3>
                <div className="mt-2 space-y-2">
                  {groupHistory.length === 0 ? (
                    <p className="g-panel-dashed px-3 py-3 text-center text-sm g-muted">今日尚無組別給分紀錄</p>
                  ) : (
                    groupHistory.map((row) => (
                      <article
                        key={row.scoreId}
                        className="rounded-xl border border-slate-600 bg-slate-800/60 px-3 py-2.5"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="rounded border border-rose-400/50 px-2 py-0.5 text-xs font-semibold text-rose-300 hover:bg-rose-950/40 disabled:opacity-50"
                            >
      ...
      {scoringModalOpen && currentSpeaker && (
        ...
        >
          <div className={`w-full max-w-sm rounded-2xl p-5 shadow-xl ${dm ? "g-modal" : "bg-white"}`}>
            <h2 className={`text-lg font-black ${dm ? "g-heading" : "text-slate-800"}`}>給分</h2>
            <p className={`mt-1 text-sm ${dm ? "g-muted" : "text-slate-600"}`}>
              發言代表：{currentSpeaker.studentName}（{currentSpeaker.studentNo}）
            </p>
            {scoreMode === "group" ? (
              <div
                className={
                  dm
                    ? "mt-2 rounded-xl border border-indigo-500/30 bg-indigo-950/50 px-3 py-2 text-xs text-indigo-100"
                    : "mt-2 rounded-xl border border-indigo-100 bg-indigo-50/80 px-3 py-2 text-xs text-slate-700"
                }
              >
                <p className="font-bold text-indigo-300">
                  此為老師評分。送出後各組組長會收到評分視窗；單次合計＝老師分數＋各組組長分數加總（上限 100 分），並計入發言組「組別計分」（與個人計分分開）。
                </p>
              </div>
            ) : (
              <p className={`mt-2 text-xs ${dm ? "g-muted" : "text-slate-500"}`}>個人給分：僅記錄於發言代表。</p>
            )}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => void closeScoringModal()}
                className={
                  dm
                    ? "rounded-xl border border-slate-500 px-4 py-2 text-sm font-semibold text-slate-200 hover:bg-slate-800"
                    : "rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"
                }
              >
                取消
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              確定要刪除這筆給分嗎？
              <br />
              {"groupKey" in pendingDeleteScore
                ? `${pendingDeleteScore.groupName} +${pendingDeleteScore.score} 分（組別計分）`
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  await handleDeleteScore(target.scoreId, "groupKey" in target);
                }}
